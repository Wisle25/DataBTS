...
@section('content')
    ...
    @component('components.table.index', [
        ...
        @endphp
        @foreach ($KuesionerData as $index => $kuesioner)
            <tr class="hover:bg-gray-100 cursor-pointer" onclick="showDetails({{ json_encode($kuesioner) }})">
                <td class="px-4 py-2 text-center">{{ $index + 1 }}</td>
                <td class="px-4 py-2 text-center">{{ $kuesioner['pertanyaan'] }}</td>
                <td class="px-4 py-2 text-center" onclick="event.stopPropagation();">
                    <div class="flex justify-center">

                        <a href='{{ url('kuesioner/edit') }}' class="text-teal-700 hover:text-white hover:bg-teal-600 p-1">
                        ...
                        <form onsubmit="return confirm('Yakin akan menghapus data?')" class='d-inline ml-1 mr-2'
                            action="{{ url('kuesioner/') }}" method="post">
                            @csrf
                            @method('DELETE')
    ...
    <script>
        ...
        <tr>
        <td class="px-6 py-1">Pertanyaan</td>
        <td class="px-14 py-1">${data.pertanyaan}</td>
        </tr>
        <tr>
