...
@include('components.allert.success')
    ...
    <div class="container mx-auto p-4">
        ...
        <div class="mt-4 flex">
            <a href="{{ route('profile.edit') }}" class="mx-3 px-3 py-1  bg-blue-500 text-white rounded-lg text-[15px] cursor-pointer active:scale-[.97]">Edit Profile</a>
            <form action="{{ route('profile.delete') }}" method="POST" class="ml-2 inline-block">
                @csrf
                @method('DELETE')
                <button type="submit" class="mx-1 px-2 py-1  bg-red-500 text-white rounded-lg text-[15px] cursor-pointer active:scale-[.97]" onclick="return confirm('Are you sure you want to delete your profile? This action cannot be undone.')">Delete Profile</button>
            </form>
        </div>
    </div>
@endsection
